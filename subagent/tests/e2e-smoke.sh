...
if [ ! -f "$REC" ]; then
	echo "FAIL: no record.json under $RUN_DIR"
	exit 1
fi
grep -q '"status": "completed"' "$REC" || {
	echo "FAIL: run not completed"
	exit 1
}
grep -q '"exitCode": 0' "$REC" || {
	echo "FAIL: exitCode != 0"
	exit 1
}
CHILD="$(ls "$RUN_DIR"/*.jsonl 2>/dev/null | head -1)"
if [ -z "$CHILD" ]; then
	echo "FAIL: no child session file"
	exit 1
fi
...
grep -q '"subagent-session"' "$PS" || {
	echo "FAIL: no subagent-session pointer in parent"
	exit 1
}
...
if [ "$RUNS" -lt 4 ]; then
	echo "FAIL: expected >=4 run dirs, have $RUNS"
	exit 1
fi
...
{"runId":"sg-orphan-smoke","agent":"worker","agentSource":"user","task":"crashed","model":"m","mode":"single","parentSessionId":"p","parentSessionFile":"/p","status":"running","startedAt":$(($(date +%s) * 1000 - 5 * 60000)),"usage":{},"pid":4194304}
JSON
"$PI" --session-dir "$WORK/parent-sessions" --name e2e-orphan -p "Reply EXACTLY with ORPHAN_OK" || exit 1
grep -q '"status": "orphaned"' "$ORPHAN_DIR/record.json" || {
	echo "FAIL: orphan not reconciled"
	exit 1
}
