...
let passed = 0;
let failed = 0;
const fail = (n, m) => { failed++; console.log(`  ✗ ${n}: ${m}`); };
...
const base = (o = {}) => ({
	...
	usage: { input: 1, output: 2, cacheRead: 0, cacheWrite: 0, cost: 0.001, contextTokens: 100, turns: 2 },
	...o,
});
...
console.log("== resolveStoreDir ==");
{
	...
	if (r1 === "/Users/hllj/.pi/agent/subagents") ok("derived from session dir -> <root>/subagents");
...
console.log("== record + list + get + end ==");
{
	...
	if (rec && rec.status === "running" && rec.usage.turns === 2) ok("getRun full record");
	else fail("getRun", "n/a");
	recordEnd({ ...base(), status: "completed", endedAt: Date.now(), durationMs: 1500, exitCode: 0 }, STORE);
	const ended = getRun("run-x", STORE);
	if (ended?.status === "completed" && ended.durationMs === 1500) ok("recordEnd atomic rewrite");
	else fail("recordEnd", ended?.status);
	const list = listRuns({}, STORE);
	if (list.length === 1 && list[0].runId === "run-x") ok("listRuns newest-first");
	...
	if (listRuns({ since: Date.now() + 1 }, STORE).length === 0) ok("since filter");
...
console.log("== atomicity / corrupt skip ==");
{
	...
	if (listRuns({}, STORE).some((r) => r.runId === "run-corrupt")) fail("corrupt", "listed");
	else ok("corrupt record skipped");
}

console.log("== normalization regression: missing usage fields must not crash ==");
{
	...
	else if (typeof bare.usage.cost !== "number" || typeof bare.usage.turns !== "number") {
		fail("bare normalized", `usage.cost=${bare.usage.cost} turns=${bare.usage.turns}`);
	...
	if (Array.isArray(all) && all.length >= 1) ok("listRuns no longer crashes on bare records");
	else fail("listRuns bare", all?.length);
	if (Array.isArray(all) && typeof all[0]?.usage?.cost === "number") ok("listed bare record has numeric cost");
...
console.log("== reconcileOrphans ==");
{
	...
	recordStart(base({ runId: "o-nopid-fresh", startedAt: Date.now() - 1000 }), STORE);
	recordStart(base({ runId: "o-nopid-stale", startedAt: Date.now() - 11 * 60 * 1000 }), STORE);
	recordStart(base({ runId: "o-pid-hardstale", pid: deadPid, startedAt: Date.now() - 25 * 60 * 60 * 1000 }), STORE);
	const orphaned = reconcileOrphans(STORE);
	if (getRun("o-dead", STORE)?.status === "orphaned") ok("dead pid -> orphaned");
	else fail("dead pid");
	if (getRun("o-live", STORE)?.status === "running") ok("live pid stays running");
	else fail("live pid");
	if (getRun("o-nopid-fresh", STORE)?.status === "running") ok("fresh no-pid stays running");
	else fail("fresh no-pid");
	if (getRun("o-nopid-stale", STORE)?.status === "orphaned") ok("stale no-pid (>10m) orphaned");
	else fail("stale no-pid");
	if (getRun("o-pid-hardstale", STORE)?.status === "orphaned") ok("24h hard window orphans stale running");
...
{
	const now = Date.now();
	recordStart(base({ runId: "p-old1", startedAt: now - 20 * 86400000, status: "completed" }), STORE);
	recordStart(base({ runId: "p-old2", startedAt: now - 20 * 86400000, status: "failed" }), STORE);
	recordStart(base({ runId: "p-hardstale-live-pid", pid: 4194304, startedAt: now - 20 * 86400000, status: "running" }), STORE);
	const child = spawn("sleep", ["30"], { stdio: "ignore" });
	recordStart(base({ runId: "p-live", pid: child.pid, startedAt: now - 1000, status: "running" }), STORE);
	pruneStore(14, 100, STORE);
	if (!getRun("p-old1", STORE) && !getRun("p-old2", STORE)) ok("run >14d removed");
	else fail("age remove");
	if (!getRun("p-hardstale-live-pid", STORE)) ok(">14d running-with-pid prunable (not live)");
...
console.log("== pruneStore maxRuns cap (isolated) ==");
{
	...
	const now = Date.now();
	for (let i = 1; i <= 8; i++) recordStart(base({ runId: `c-${i}`, startedAt: now - i * 60000, status: "completed" }), CAP);
...
console.log("== pruneStore env retention override (isolated) ==");
{
	...
	const now = Date.now();
	recordStart(base({ runId: "rt-old1", startedAt: now - 2 * 86400000, status: "completed" }), RT);
	recordStart(base({ runId: "rt-old2", startedAt: now - 2 * 86400000, status: "failed" }), RT);
	recordStart(base({ runId: "rt-fresh", startedAt: now - 1000, status: "completed" }), RT);
...
console.log("== readRunTranscript (SessionManager.open) ==");
{
	...
	const header = JSON.stringify({ type: "session", version: 3, id: "01abc", timestamp: "2026-08-21T00:00:00.000Z", cwd: "/tmp" });
	const m1 = JSON.stringify({ type: "message", id: "m1", parentId: null, timestamp: "2026-08-21T00:00:00.001Z", message: { role: "user", content: [{ type: "text", text: "hi" }] } });
	const m2 = JSON.stringify({ type: "message", id: "m2", parentId: "m1", timestamp: "2026-08-21T00:00:00.002Z", message: { role: "assistant", content: [{ type: "text", text: "found 3 files" }] } });
	...
	if (!t) fail("transcript", "undefined");
		...
		if (t.messages[0].role === "user" && t.messages[1].role === "assistant") ok("roles ordered");
		else fail("roles");
	}
}
...
console.log("== readRunTranscript JSONL fallback ==");
{
	...
	const hdr = JSON.stringify({ type: "session", version: 3, id: "01xyz", timestamp: "2026-08-21T00:00:00.000Z", cwd: "/tmp" });
	const msg = JSON.stringify({ type: "message", id: "f1", parentId: null, timestamp: "2026-08-21T00:00:00.001Z", message: { role: "user", content: [{ type: "text", text: "hi" }] } });
